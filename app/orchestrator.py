...
from app import agents, memory, sync, web_search
...
```json
{
  ...
  "search": "웹 검색이 필요하면 검색할 키워드 (필요없으면 빈 문자열)",
  "tasks": [
...
5. "최신", "트렌드", "분석", "경쟁사", "시장", "뉴스" 등의 키워드가 포함되면 search에 적절한 검색어를 넣으세요.
"""
...
def _fallback_distribute(user_message: str) -> dict:
    ...
    return {
        ...
        "search": user_message[:50] if any(k in msg for k in ["최신", "트렌드", "분석", "경쟁사", "시장", "뉴스"]) else "",
        "tasks": tasks,
    }
...
async def run_workflow(user_message: str, target_agent: str = None) -> AsyncGenerator[str, None]:
    ...
    # 2. 웹 검색 (필요한 경우)
    search_context = ""
    search_query = distribution.get("search", "")

    # CEO가 검색 키워드를 지정하지 않았어도 키워드 기반 자동 감지
    if not search_query:
        search_keywords = ["최신", "트렌드", "분석", "경쟁사", "시장", "뉴스", "현재", "요즘", "2024", "2025", "2026"]
        if any(k in user_message for k in search_keywords):
            search_query = user_message[:50]

    if search_query:
        yield _sse_event("web_searching", {"query": search_query})
        try:
            results = await web_search.search_web(search_query, max_results=5)
            search_context = web_search.format_search_results(results, search_query)
            yield _sse_event("search_done", {
                "query": search_query,
                "count": len(results),
            })
        except Exception as e:
    ...
    for t in enriched_tasks:
        ...
        # 검색 결과가 있으면 작업 지시에 포함
        agent_task = task_desc
        if search_context:
            agent_task = f"{task_desc}\n\n---\n\n{search_context}"
        ...
        full_response = ""
        try:
            async for chunk in agents.call_agent_stream(aid, user_message, task=agent_task):
                full_response += chunk
                yield _sse_event("agent_chunk", {
                    "agent_id": aid,
                    "content": chunk,
