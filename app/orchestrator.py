...
def _build_report_prompt(outputs: dict, original_message: str) -> str:
    ...
    for aid, content in outputs.items():
        ...
        name = meta.get("name", aid)
        emoji = meta.get("emoji", "")
        sections.append(f"### {emoji} {name} 산출물:\n{content[:2000]}")

    all_outputs_text = "\n\n".join(sections)

    return f"""사용자의 원 명령: "{original_message}"

아래는 각 에이전트의 산출물입니다. 이를 종합하여 CEO 보고서를 작성하세요.

{all_outputs_text}

## 보고서 형식
1. **✅ 완료된 작업** — 각 에이전트가 무엇을 완료했는지 요약
2. **🚀 다음 액션 (Top 3)** — 가장 우선순위가 높은 다음 행동 3가지
3. **💡 인사이트** — 전체 산출물에서 발견한 핵심 통찰
"""
